...
def _generate_sync(loc: dict, guest_bytes: bytes, outfit: str):
    ...
    # Для face-swap — СЫРОЕ фото гостя целиком, ДО GFPGAN (GFPGAN «причёсывает»
    # лицо и снижает сходство). Именно целиком, а не тесный кроп: 27.07.2026 обе
    # свап-модели стали отвечать «No face found» — их детектор не находил лицо в
    # растянутом кропе, при том что наш insightface на полном кадре находил его
